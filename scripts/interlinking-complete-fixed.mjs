...
const dryRun = false; // Cambiar a false para aplicar cambios
...
function generateLinkText(relatedFile, context) {
  const title = (relatedFile.file?.fileName || relatedFile.fileName || 'unknown')
    .replace(/-/g, ' ')
  ...
  for (let i = 0; i < lines.length && linksAdded < maxLinks; i++) {
    ...
    if (headingMatch) {
      ...
      if (level >= 2 && level <= 3 && linksAdded < maxLinks && linksAdded < relatedFiles.length) {
        ...
        newLines.push('');
        newLines.push(`> **${linkText}**: [${(relatedFile.file?.fileName || relatedFile.fileName || 'unknown').replace(/-/g, ' ')}](${url})`);
        linksAdded++;
      } else {
        newLines.push(line);
      }
